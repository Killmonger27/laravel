...
export default function GuichetInterface({ 
    ...
    const [ticketsEnAttente, setTicketsEnAttente] = useState(initialTicketsEnAttente);
    ...
    const appellerProchain = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                setTicketsEnAttente(prev => prev.filter(t => t.id !== response.data.ticket.id));
            ...
            afficherNotification(error.response?.data?.error || 'Erreur lors de l\'appel', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const commencerTicket = async (ticketId) => {
        ...
        setLoading(true);
        try {
            const response = await axios.post(route('guichet.commencer', ticketId));
            
            if (response.data.success) {
                setTicketEnCours(response.data.ticket);
                setTicketsAppeles(prev => prev.filter(t => t.id !== ticketId));
                afficherNotification('Traitement commencé', 'success');
            }
        } catch (error) {
            afficherNotification(error.response?.data?.error || 'Erreur lors du démarrage', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const terminerTicket = async (ticketId) => {
        ...
        setLoading(true);
        try {
            const response = await axios.post(route('guichet.terminer', ticketId));
            ...
            afficherNotification(error.response?.data?.error || 'Erreur lors de la finalisation', 'error');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-blue-600 text-white p-6">
                <div className="max-w-7xl mx-auto">
                    <div className="flex justify-between items-center">
                        <div>
                            <h1 className="text-3xl font-bold">Guichet {guichet.numero}</h1>
                            <p className="text-blue-100">Service: {service.nom}</p>
                            <p className="text-blue-100">Agent: {agent.name} ({agent.matricule})</p>
                        </div>
                        <div className="text-right">
                            <div className="text-sm text-blue-100">Statut du guichet</div>
                            <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                                guichet.statut === 'libre' ? 'bg-green-500' : 'bg-orange-500'
                            }`}>
                                {guichet.statut === 'libre' ? 'Libre' : 'Occupé'}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
                            <h2 className="text-xl font-semibold mb-4">Ticket en cours</h2>
                            ...
                            {ticketEnCours ? (
                                <div className="border-l-4 border-blue-500 pl-4 py-4 bg-blue-50 rounded">
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            ...
                                            <p className="text-gray-600">Client: {ticketEnCours.nom_client}</p>
                                            {ticketEnCours.telephone_client && (
                                                <p className="text-gray-600">Tél: {ticketEnCours.telephone_client}</p>
                                            )}
                                        </div>
                                        <div className="text-right text-sm text-gray-500">
                                            <p>Appelé: {formatHeure(ticketEnCours.heure_appel)}</p>
                                            {ticketEnCours.heure_debut && (
                                                <p>Commencé: {formatHeure(ticketEnCours.heure_debut)}</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-3">
                                        <button
                                            onClick={() => terminerTicket(ticketEnCours.id)}
                                            disabled={loading}
                                ...
                                <div className="text-center py-8 text-gray-500">
                                    ...
                                    <p className="text-sm">Appelez le prochain client pour commencer</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-white rounded-lg shadow-md p-6">
                            <div className="flex justify-between items-center mb-4">
                                <h2 className="text-xl font-semibold">Tickets appelés</h2>
                                <button
                                    onClick={appellerProchain}
                                    disabled={loading || ticketsEnAttente.length === 0}
                            ...
                            {ticketsAppeles.length > 0 ? (
                                        ...
                                        >
                                            <div className="flex justify-between items-center">
                                                <div>
                                                    ...
                                                    <p className="text-gray-600">{ticket.nom_client}</p>
                                                    <p className="text-sm text-gray-500">
                                                        Appelé: {formatHeure(ticket.heure_appel)}
                                                    </p>
                                                </div>
                                                <button
                                                    onClick={() => commencerTicket(ticket.id)}
                                                    disabled={loading || ticketEnCours}
                                ...
                                <p className="text-gray-500 text-center py-4">Aucun ticket appelé</p>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="bg-white rounded-lg shadow-md p-6">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-semibold">{statistiques.tickets_traites_aujourdhui}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>Temps moyen:</span>
                                    <span className="font-semibold">{statistiques.temps_moyen_traitement} min</span>
                                </div>
                            </div>
                        </div>
                            ...
                            {ticketsEnAttente.length > 0 ? (
                                <div className="space-y-2 max-h-64 overflow-y-auto">
                                    {ticketsEnAttente.slice(0, 10).map((ticket, index) => (
                                        ...
                                        >
                                            <div className="text-sm font-medium">{ticket.numero}</div>
                                            <div className="text-xs text-gray-500">{ticket.nom_client}</div>
                                            <div className="text-xs text-gray-400">
                                                {formatHeure(ticket.created_at)}
                                            </div>
                                        </div>
                                    ))}
                                    {ticketsEnAttente.length > 10 && (
                                        <div className="text-center text-sm text-gray-500 pt-2">
                                            ... et {ticketsEnAttente.length - 10} autres
                                        </div>
                                    )}
                                </div>
                            ) : (
